...
class CustomTrigger[T, W <: TimeWindow ] extends Trigger[T, TimeWindow] {
  //触发计算的最大数量
  ...
  @throws[Exception]
  private def fireAndPurge(window: TimeWindow, ctx: Trigger.TriggerContext) = {
    clear(window, ctx)
  ...
  override def onEventTime(time: Long, window: TimeWindow, ctx: Trigger.TriggerContext): TriggerResult = {
    if ((time >= window.maxTimestamp()) && (ctx.getPartitionedState(countStateDescriptor).get() > 0L)) {
      //还有未触发计算的数据
      ...
      println("事件时间到达最大的窗口时间，但是窗口中没有有未计算的数据，不清除窗口 不触发计算")
      TriggerResult.CONTINUE
    } else {
      TriggerResult.CONTINUE
